...
import type { CheckedState, StackProps, CheckboxProps as TMCheckboxProps } from 'tamagui'
...
function RawCheckbox({
  ...
  const Indicator = useMemo(() => {
    if (value) {
      return (
        <Icon
          name={value === 'indeterminate' ? 'AppleIcon' : 'GoogleIcon'}
          color="$iconInverse"
          size="$4"
        />
      )
  ...
  return (
    ...
    >
      ...
      {label && (
        <Label htmlFor={id} variant="$bodyLgMedium" pl="$2" paddingVertical="$2" my="$-2" {...labelProps}>
          {label}
        </Label>
      )}
    </XStack>
...
function CheckboxGroupItem({
  ...
  return <RawCheckbox label={label} value={value} disabled={disabled} onChange={handleOnChange} />
}
...
function CheckboxGroup({
  ...
  const [isAll, setAll] = useState(value.length === options.length && !value.find((v) => !v))
  ...
  return (
    <YStack>
      <RawCheckbox disabled={disabled} label={label} value={isAll} onChange={handleSelectAll} />
      <Divider />
      <ListView
        ...
        renderItem={({ item: { label: labelText, disabled: disabledElement }, index }) => (
          <CheckboxGroupItem
            key={label}
            label={labelText}
            value={value[index]}
