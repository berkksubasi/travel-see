...
export type ISortableSectionRenderInfo = (info: { section: any; index: number }) => ReactNode | null
...
export type ISortableSectionListProps = Omit<ScrollViewProps, 'contentContainerStyle'> &
  StackStyleProps & {
    ...
    getItemLayout: (item: any, index: number) => { length: number; offset: number; index: number }
...
function BaseSortableSectionList(
  {
    ...
    ListHeaderComponent,
    ListFooterComponent,
    SectionSeparatorComponent = <Stack height="$5" />,
  ...
  const rawContentContainerStyle = useStyle(contentContainerStyle as Record<string, unknown>, {
    resolveValues: 'auto',
  })

  const listHeaderStyle = useStyle(ListHeaderComponentStyle as Record<string, unknown>, {
    resolveValues: 'auto',
  })

  const listFooterStyle = useStyle(ListFooterComponentStyle as Record<string, unknown>, {
    resolveValues: 'auto',
  })
  ...
  const reloadStickyHeaderIndices = useMemo(() => {
    ...
    return sections.map(
      (section, index) =>
        (2 + (SectionSeparatorComponent ? 1 : 0) + (renderSectionFooter ? 1 : 0)) * index + 1
    )
  }, [stickySectionHeadersEnabled, sections, SectionSeparatorComponent, renderSectionFooter])

  const scrollChildList = useMemo(() => {
    const childList: [ReactNode] = [<Stack style={listHeaderStyle}>{ListHeaderComponent}</Stack>]
    ...
    childList.push(<Stack style={listFooterStyle}>{ListFooterComponent}</Stack>)
    return childList
  }, [
    ListHeaderComponent,
    ListFooterComponent,
