...
export const Alert: FC<IAlertProps> = ({ icon, title, description, closable, type, action }) => {
  ...
  return (
    <AlertFrame type={type}>
      ...
      <YStack flex={1} space="$1">
        ...
        {action ? (
          <XStack pt="$2" space="$4" alignItems="center">
            ...
            {action.secondary ? (
              <Button size="small" variant="tertiary" onPress={action.onSecondaryPress}>
                {action.secondary}
              </Button>
            ) : null}
          </XStack>
      ...
      {closable ? (
        <IconButton icon="CrossedSmallSolid" size="small" variant="tertiary" onPress={onClose} />
      ) : null}
    </AlertFrame>
  )
}
